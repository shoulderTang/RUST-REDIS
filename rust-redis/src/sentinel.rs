...
impl SentinelState {
    pub fn new(ip: String, port: u16) -> Self {
        Self {
            ...
            run_id: format!("sentinel_{}", SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis()),
            ip,
            port,
...
impl SentinelState {
    ...
    pub fn monitor_master(&self, name: &str, ip: &str, port: u16, quorum: u32) -> Result<(), String> {
        ...
        info!(master = name, ip = ip, port = port, quorum = quorum, "sentinel_monitor_added");
        Ok(())
    }
    ...
    pub fn set_master_options(&self, name: &str, options: &[(String, String)]) -> Result<(), String> {
        let masters = self.masters.read().map_err(|_| "lock error".to_string())?;
        let lock = masters.get(name).ok_or_else(|| "ERR No such master with that name".to_string())?;
            ...
            match key.as_str() {
                "down-after-milliseconds" => {
                    if let Ok(v) = val.parse::<u64>() {
                        ...
                        info!(master = name, option = "down-after-milliseconds", value = v, "sentinel_set_option");
                    } else {
                ...
                "failover-timeout" => {
                    if let Ok(v) = val.parse::<u64>() {
                        ...
                        info!(master = name, option = "failover-timeout", value = v, "sentinel_set_option");
                    } else {
                ...
                "parallel-syncs" => {
                    if let Ok(v) = val.parse::<u32>() {
                        ...
                        info!(master = name, option = "parallel-syncs", value = v, "sentinel_set_option");
                    } else {
                ...
                "quorum" => {
                    if let Ok(v) = val.parse::<u32>() {
                        master.quorum = v;
                        info!(master = name, option = "quorum", value = v, "sentinel_set_option");
                    } else {
    ...
    pub fn generate_config_text(&self) -> String {
        ...
        for (name, master) in entries {
            lines.push(format!("sentinel monitor {} {} {} {}", name, master.ip, master.port, master.quorum));
            lines.push(format!("sentinel down-after-milliseconds {} {}", name, master.down_after_period));
            lines.push(format!("sentinel failover-timeout {} {}", name, master.failover_timeout));
            lines.push(format!("sentinel parallel-syncs {} {}", name, master.parallel_syncs));
        }
    ...
    if let Ok(masters) = state.masters.read() {
        if let Some(master_lock) = masters.get(master_name) {
            if let Ok(mut master) = master_lock.write() {
                ...
                info!("Connection failure detected for master {} {}:{}", master.name, master.ip, master.port);
            }
        }
    }
}
...
async fn request_vote_from_sentinel(
    ...
    info!("Requesting vote from sentinel {}:{} for master {} (epoch: {})", 
    ...
    if vote_success {
        info!("Received vote from sentinel {}:{} for master {}", sentinel_ip, sentinel_port, master_name);
        Ok(true)
    } else {
        info!("Vote denied by sentinel {}:{} for master {}", sentinel_ip, sentinel_port, master_name);
        Ok(false)
    }
}

pub async fn elect_leader_for_master(state: &SentinelState, master_name: &str) -> Result<bool, String> {
...
pub async fn sentinel_cron(state: SentinelState) {
    ...
    let mut connections: HashMap<String, (tokio::sync::mpsc::Sender<Resp>, tokio::sync::mpsc::Sender<Resp>, String, u16)> = HashMap::new();
        ...
        for (name, ip, port) in masters_snapshot {
            ...
            if let Some((cmd_tx, _pubsub_tx, _c_ip, _c_port)) = connections.get(&name) {
                ...
                if let Some((master_ip, master_port, epoch)) = hello_info {
                    let master_epoch = {
                        let masters = state.masters.read().unwrap();
                        masters.get(&name).map(|m| m.read().unwrap().failover_epoch).unwrap_or(0)
                    };
                    let hello_msg = format!("{},{},{},{},{},{},{},{}",
                        state.ip, state.port, state.run_id, epoch,
                        name, master_ip, master_port, master_epoch);
            ...
            // Check SDOWN state and perform ODOWN check
            {
                        ...
                        if let Ok(mut master) = master_lock.write() {
                            ...
                            if now > master.last_pong_time + master.down_after_period {
                                if master.s_down_since_time == 0 {
                                    ...
                                    warn!("+sdown master {} {}:{}", master.name, master.ip, master.port);
                                }
                                is_sdown = true;
                            } else {
                                if master.s_down_since_time > 0 {
                ...
                if is_sdown {
                    ...
                    for (_sid, ip, port) in sentinels_to_ask {
                        if ask_sentinel_is_master_down(&ip, port, &master_ip, master_port, &run_id).await {
                            votes += 1;
                        }
                    }
                    ...
                    if votes >= quorum {
                        ...
                        // For now we log it.
                        warn!("+odown master {} {}:{} #quorum {}/{}", name, master_ip, master_port, votes, quorum);
...
async fn failover_state_machine(state: SentinelState, master_name: String) {
    ...
    match elect_leader_for_master(&state, &master_name).await {
        ...
        Ok(false) => {
            info!("Failed to elect leader for master {} - insufficient quorum", master_name);
            return;
        }
        Err(e) => {
    ...
    if !is_current_leader(&state, &master_name) {
        error!("No longer the leader for master {} - aborting failover", master_name);
        return;
    }
    ...
    if !old_master_ip.is_empty() && old_master_port > 0 {
        if let Ok(tx) = connect_to_master(state.clone(), master_name.clone(), &old_master_ip, old_master_port).await {
            let info_cmd = Resp::Array(Some(vec![Resp::BulkString(Some(Bytes::from("INFO")))]));
    ...
    if let Some((ip, port)) = selected_slave {
        ...
        let parallel = {
            let masters = state.masters.read().unwrap();
            masters.get(&master_name).map(|m| m.read().unwrap().parallel_syncs).unwrap_or(1).max(1)
        } as usize;
        let mut idx = 0;
        while idx < slaves_to_reconf.len() {
            ...
            for (s_ip, s_port) in batch {
                if let Ok(tx) = connect_to_master(state.clone(), master_name.clone(), s_ip, *s_port).await {
                    let cmd = Resp::Array(Some(vec![
                        Resp::BulkString(Some(Bytes::from("REPLICAOF"))),
        ...
        warn!("+switch-master {} {} {} {} {}", master_name, old_master_ip, old_master_port, ip, port);
        let _ = state.publish_notification("+switch-master", format!("{} {} {} {} {}", master_name, old_master_ip, old_master_port, ip, port));
        
    } else {
        error!("Failover failed: No healthy slave found for {}", master_name);
...
async fn ask_sentinel_is_master_down(sentinel_ip: &str, sentinel_port: u16, master_ip: &str, master_port: u16, _run_id: &str) -> bool {
...
async fn connect_to_master_pubsub(state: SentinelState, name: String, ip: &str, port: u16) -> Result<tokio::sync::mpsc::Sender<Resp>, Box<dyn std::error::Error + Send + Sync>> {
            ...
            match read_frame(&mut reader).await {
                Ok(Some(resp)) => {
                    if let Resp::Array(Some(items)) = resp {
                        if items.len() == 3 {
                            if let (Resp::BulkString(Some(channel)), Resp::BulkString(Some(message))) = (&items[1], &items[2]) {
                                let channel = String::from_utf8_lossy(channel);
                                if channel == "__sentinel__:hello" {
                                    let msg = String::from_utf8_lossy(message);
                                    handle_sentinel_hello_message(&masters_clone, &name_clone, &msg);
                                }
                            }
                        }
                    }
        ...
        if let Some(master_lock) = masters.get(expected_master) {
            if let Ok(mut master) = master_lock.write() {
                ...
                if let Some(s) = master.sentinels.get_mut(&sentinel_runid) {
                    ...
                    debug!(runid = s.run_id.as_str(), ip = s.ip.as_str(), port = s.port, epoch = s.leader_epoch, "sentinel_hello_refresh");
                } else {
...
pub fn check_quorum_for_master(
    ...
    let lock = masters.get(master_name).ok_or_else(|| "ERR No such master with that name".to_string())?;
    ...
    info!(master = master_name, total = total, reachable = reachable, majority = majority, quorum = quorum, ok = ok, "sentinel_ckquorum");
...
pub async fn start_manual_failover(state: &SentinelState, master_name: &str) -> Result<(), String> {
    ...
    match elect_leader_for_master(state, master_name).await {
        Ok(true) => {
            info!("Elected as leader for manual failover of master {}", master_name);
        }
        Ok(false) => {
            return Err(format!("Failed to elect leader for master {} - insufficient quorum", master_name));
        }
        Err(e) => {
            return Err(format!("Leader election failed for master {}: {}", master_name, e));
        }
    }
...
async fn connect_to_master(state: SentinelState, name: String, ip: &str, port: u16) -> Result<tokio::sync::mpsc::Sender<Resp>, Box<dyn std::error::Error + Send + Sync>> {
    let addr = format!("{}:{}", ip, port);
    let stream = TcpStream::connect(addr).await?;
    let (read_half, write_half) = stream.into_split();
    let mut reader = BufReader::new(read_half);
                                 ...
                                 if let Ok(masters) = masters_clone.read() {
                                     if let Some(master_lock) = masters.get(&name_clone) {
                                         if let Ok(mut master) = master_lock.write() {
                                             ...
                                             if master.s_down_since_time > 0 {
                                                 info!("Master {} is back online (PONG received)", name_clone);
                                                 master.s_down_since_time = 0;
                                             }
                                         }
                                     }
                                     ...
                                     if let Some(master_lock) = masters.get(&name_clone) {
                                         if let Ok(mut master) = master_lock.write() {
                                              ...
                                              master.info_cache = Some(InfoCache {
                                                  last_refresh_time: SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis() as u64,
                                                  raw: s_owned.clone(),
                                              });
                                         }
                                     }
