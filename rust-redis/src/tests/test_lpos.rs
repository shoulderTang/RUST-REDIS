...
async fn test_lpos() {
    ...
    let res = run_cmd(
        vec!["LPOS", "mylist", "a", "RANK", "-1", "COUNT", "2"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
    ...
    let res = run_cmd(
        vec!["LPOS", "mylist", "a", "RANK", "2", "MAXLEN", "2"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
