...
async fn test_sentinel_failover_integration() {
    ...
    run_cmd(vec!["SET", "test_key", "test_value"], &mut master_conn, &master_ctx).await;
    ...
    // Set up slave role
    run_cmd(vec!["REPLICAOF", "127.0.0.1", "6379"], &mut slave_conn, &slave_ctx).await;
    ...
    run_cmd(vec!["REPLICAOF", "127.0.0.1", "6380"], &mut master_conn, &master_ctx).await;
...
async fn test_sentinel_monitoring_simulation() {
    ...
    run_cmd(vec!["REPLICAOF", "NO", "ONE"], &mut master_conn, &master_ctx).await;
