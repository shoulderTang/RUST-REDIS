...
async fn test_xpending_basic() {
    ...
    // 6. XPENDING detailed
    let res = run_cmd(
        vec!["XPENDING", "mystream", "mygroup", "-", "+", "10"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
...
async fn test_xpending_idle() {
    ...
    let res = run_cmd(
        vec![
            "XPENDING", "mystream", "mygroup", "IDLE", "1000", "-", "+", "10",
        ],
        &mut conn_ctx,
        &server_ctx,
    )
    ...
    let res = run_cmd(
        vec![
            "XPENDING", "mystream", "mygroup", "IDLE", "0", "-", "+", "10",
        ],
        &mut conn_ctx,
        &server_ctx,
    )
...
async fn test_xpending_errors() {
    ...
    // NOGROUP error
    let res = run_cmd(
        vec!["XPENDING", "nosuchstream", "mygroup"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
    ...
    .await;
    let res = run_cmd(
        vec!["XPENDING", "mystream", "nosuchgroup"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
