...
mod tests {
    ...
    async fn test_unlink_basic() {
        ...
        assert_eq!(
            run_cmd(vec!["EXISTS", "k1"], &mut conn_ctx, &server_ctx).await,
            Resp::Integer(0)
        );
        assert_eq!(
            run_cmd(vec!["EXISTS", "k2"], &mut conn_ctx, &server_ctx).await,
            Resp::Integer(0)
        );
        assert_eq!(
            run_cmd(vec!["EXISTS", "k3"], &mut conn_ctx, &server_ctx).await,
            Resp::Integer(1)
        );
    }
