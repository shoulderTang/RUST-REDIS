...
async fn test_bitfield() {
    ...
    let res = run_cmd(
        vec!["BITFIELD", "mykey", "INCRBY", "i8", "0", "30"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
    ...
    // Test SAT overflow
    run_cmd(
        vec!["BITFIELD", "mykey", "SET", "i8", "0", "120"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
    let res = run_cmd(
        vec![
            "BITFIELD", "mykey", "OVERFLOW", "SAT", "INCRBY", "i8", "0", "10",
        ],
        &mut conn_ctx,
        &server_ctx,
    )
