...
async fn test_replconf_listening_port() {
    ...
    let res = run_cmd(vec!["REPLCONF", "LISTENING-PORT", "6381"], &mut conn_ctx, &server_ctx).await;
...
async fn test_replconf_listening_port_invalid() {
    ...
    let res = run_cmd(vec!["REPLCONF", "LISTENING-PORT", "99999"], &mut conn_ctx, &server_ctx).await;
    assert_eq!(res, Resp::SimpleString(Bytes::from("OK")));
...
async fn test_replconf_listening_port_zero() {
    ...
    let res = run_cmd(vec!["REPLCONF", "LISTENING-PORT", "0"], &mut conn_ctx, &server_ctx).await;
    assert_eq!(res, Resp::SimpleString(Bytes::from("OK")));
