...
async fn test_smove() {
    ...
    let res = run_cmd(
        vec!["SMOVE", "source", "dest", "non_exist"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
    ...
    let res = run_cmd(
        vec!["SMOVE", "non_exist_key", "dest", "m1"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
    ...
    let res = run_cmd(
        vec!["SMOVE", "source", "wrong_dest", "m2"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
    ...
    assert_eq!(
        res,
        Resp::Integer(0),
        "Source key should be removed when empty"
    );
}
