...
mod tests {
    ...
    async fn test_copy() {
        ...
        assert_eq!(
            run_cmd(vec!["GET", "k2"], &mut conn_ctx, &server_ctx).await,
        ...
        conn_ctx.db_index = 1;
        assert_eq!(
            run_cmd(vec!["GET", "k3"], &mut conn_ctx, &server_ctx).await,
            Resp::BulkString(Some(Bytes::from("v1")))
        );
    }
    ...
    async fn test_server_cmds() {
        ...
        match res {
            Resp::Array(Some(arr)) => {
                assert_eq!(arr[0], Resp::BulkString(Some(Bytes::from("master"))))
            }
