...
async fn test_zrangebyscore() {
    ...
    // 2. WITHSCORES
    let res = run_cmd(
        vec!["ZRANGEBYSCORE", "myzset", "2", "3", "WITHSCORES"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
    ...
    // 3. LIMIT
    let res = run_cmd(
        vec!["ZRANGEBYSCORE", "myzset", "1", "5", "LIMIT", "1", "2"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
...
async fn test_zrangebylex() {
    ...
    // 2. LIMIT
    let res = run_cmd(
        vec!["ZRANGEBYLEX", "myzset", "-", "+", "LIMIT", "1", "2"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
