...
async fn test_zdiff() {
    ...
    let res = run_cmd(
        vec!["ZDIFF", "2", "zset1", "zset2", "WITHSCORES"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
    ...
    // 3. ZDIFFSTORE
    let res = run_cmd(
        vec!["ZDIFFSTORE", "out", "2", "zset1", "zset2"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
