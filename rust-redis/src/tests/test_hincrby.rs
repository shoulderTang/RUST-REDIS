...
async fn test_hincrby() {
    ...
    // 6. Value not integer
    run_cmd(
        vec!["HSET", "myhash", "field3", "notanumber"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
    let res = run_cmd(
        vec!["HINCRBY", "myhash", "field3", "1"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
...
async fn test_hincrbyfloat() {
    ...
    // 1. New key
    let res = run_cmd(
        vec!["HINCRBYFLOAT", "myfloat", "field1", "10.5"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
    ...
    // 2. Increment
    let res = run_cmd(
        vec!["HINCRBYFLOAT", "myfloat", "field1", "0.1"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
    ...
    // 3. Negative
    let res = run_cmd(
        vec!["HINCRBYFLOAT", "myfloat", "field1", "-5"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
    ...
    // 4. Scientific
    let res = run_cmd(
        vec!["HINCRBYFLOAT", "myfloat", "field1", "2.0e2"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
    ...
    run_cmd(vec!["SET", "mystring", "foo"], &mut conn_ctx, &server_ctx).await;
    let res = run_cmd(
        vec!["HINCRBYFLOAT", "mystring", "field", "1.0"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
    ...
    // 6. Value not float
    run_cmd(
        vec!["HSET", "myfloat", "field3", "notanumber"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
    let res = run_cmd(
        vec!["HINCRBYFLOAT", "myfloat", "field3", "1.0"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
