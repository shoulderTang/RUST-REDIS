...
mod tests {
    ...
    async fn test_stream_aof_log_replay() {
        ...
        macro_rules! run_and_log {
            ($args:expr, $conn:expr, $server:expr) => {{
                let req = Resp::Array(Some(
                    $args
                        .iter()
                        .map(|s| Resp::BulkString(Some(Bytes::from(s.to_string()))))
                        .collect(),
                ));
        ...
        // 1. XADD
        let log1 = run_and_log!(
            vec!["XADD", "mystream", "*", "name", "alice"],
            &mut conn_ctx,
            &server_ctx
        )
        .unwrap();
        ...
        let log3 = run_and_log!(
            vec!["XADD", "mystream", "*", "name", "bob"],
            &mut conn_ctx,
            &server_ctx
        )
        .unwrap();
        ...
        let log5 = run_and_log!(
            vec!["XACK", "mystream", "mygroup", &stream_id_bob],
            &mut conn_ctx,
            &server_ctx
        )
        .unwrap();
        ...
        let alice_id = {
            ...
            let stream_ids: Vec<String> = match &entry.value {
                Value::Stream(s) => s
                    .range(&StreamID::new(0, 0), &StreamID::new(u64::MAX, u64::MAX))
                    .iter()
        ...
        let log6 = run_and_log!(
            vec!["XDEL", "mystream", &alice_id],
            &mut conn_ctx,
            &server_ctx
        )
        .unwrap();
