...
async fn test_zrandmember() {
    ...
    run_cmd(
        vec!["ZADD", "myzset", "10", "a", "20", "b", "30", "c"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
    ...
    let res = run_cmd(
        vec!["ZRANDMEMBER", "myzset", "2", "WITHSCORES"],
        &mut conn_ctx,
        &server_ctx,
    )
    .await;
