...
use sentinel::{FailoverState, MasterInstance, SentinelState, SlaveInstance};
...
async fn test_failover_state_transitions() {
    ...
    // Add a master
    {
        let mut masters = state.masters.write().unwrap();
        let master =
            MasterInstance::new("testmaster".to_string(), "127.0.0.1".to_string(), 6379, 2);
...
async fn test_failover_reset() {
    ...
    // Add a master with some state
    {
        let mut masters = state.masters.write().unwrap();
        let mut master =
            MasterInstance::new("resetmaster".to_string(), "127.0.0.1".to_string(), 6379, 2);
        master.failover_state = FailoverState::InProgress;
        master.failover_start_time = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
...
async fn test_failover_with_unhealthy_slaves() {
    ...
    // Add a master with unhealthy slaves
    {
        let mut masters = state.masters.write().unwrap();
        let mut master =
            MasterInstance::new("unhealthy".to_string(), "127.0.0.1".to_string(), 6379, 2);
        ...
        let slave = SlaveInstance {
            ...
            s_down_since_time: SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap()
                .as_millis() as u64,
        };
