...
mod tests {
    ...
    async fn test_xadd_nomkstream() {
        ...
        let res = run_cmd(
            vec!["XADD", "mystream", "NOMKSTREAM", "*", "f1", "v1"],
            &mut conn_ctx,
            &server_ctx,
        )
        .await;
        ...
        let res = run_cmd(
            vec!["XADD", "mystream", "NOMKSTREAM", "*", "f2", "v2"],
            &mut conn_ctx,
            &server_ctx,
        )
        .await;
    ...
    async fn test_xgroup_createconsumer() {
        ...
        // Check info
        let res = run_cmd(
            vec!["XINFO", "CONSUMERS", "mystream", "mygroup"],
            &mut conn_ctx,
            &server_ctx,
        )
        .await;
    ...
    async fn test_command_subcommands() {
        ...
        // COMMAND GETKEYS
        let res = run_cmd(
            vec!["COMMAND", "GETKEYS", "MGET", "k1", "k2", "k3"],
            &mut conn_ctx,
            &server_ctx,
        )
        .await;
