...
async fn test_hash_ops() {
    ...
    // HSET hash f1 v1 -> 1
    let res = run_cmd(vec!["HSET", "hash", "f1", "v1"], &mut conn_ctx, &server_ctx).await;
    match res {
    ...
    // HLEN hash -> 3
    let res = run_cmd(vec!["HLEN", "hash"], &mut conn_ctx, &server_ctx).await;
    match res {
    ...
    // HDEL hash f1 -> 1
    let res = run_cmd(vec!["HDEL", "hash", "f1"], &mut conn_ctx, &server_ctx).await;
    match res {
        Resp::Integer(i) => assert_eq!(i, 1),
        _ => panic!("expected Integer(1)"),
    }
